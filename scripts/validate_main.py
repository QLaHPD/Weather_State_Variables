...
from weather_state_variables.training import validate_main_model


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run standalone validation for the FuXi-style forecast model.")
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--checkpoint",
        default=None,
        help="Optional checkpoint path. Defaults to train_main.output_dir/train_main.checkpoint_name from config.",
    )
    parser.add_argument(
        "--split",
        choices=("val", "train"),
    ...
    parser.add_argument(
        "--print-model-summary",
        action="store_true",
        help="Print the model summary before loading the checkpoint and running validation.",
    )
    parser.add_argument(
        "--save-rollout-plots",
        action="store_true",
        help="Save autoregressive rollout plots for a small number of validation samples.",
    )
    parser.add_argument(
        "--rollout-output-dir",
        default=None,
